...
async function main() {
  ...
  // --- CHECK 1: FILES EXIST ---
  if (!fs.existsSync(MODEL_PATH)) throw new Error(`Missing model at ${MODEL_PATH}`);
  if (!fs.existsSync(SKIPLIST_PATH)) throw new Error(`Missing skiplist at ${SKIPLIST_PATH}`);
  ...
  if (qCheck === 50368 && dCheck === 50369) {
      ...
      console.error(`❌ Tokenizer Map Mismatch! Found [Q]->${qCheck}, [D]->${dCheck}`);
  ...
  const batchIds = [BigInt(CLS_ID), BigInt(Q_ID), BigInt(1234), BigInt(tokenizer.sep_token_id ?? 50282)];
  ...
  const docLen = 1000;
  const docIds = new Array(docLen).fill(0).map(() => Math.floor(Math.random() * 50000));
